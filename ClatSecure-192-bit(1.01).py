...
def generate_aes_key(password: bytes, salt: bytes = None, iterations: int = 1000000):
    if salt is None:
        salt = os.urandom(16)
...
def generate_and_print_keys(password: bytes, iterations: int = 1000000):
    for _ in range(100):
        try:
            generated_key, used_salt = generate_aes_key(password, iterations=iterations)
...
if __name__ == "__main__":
    ...
    print("   ██████╗ ███████╗  ██║  ██║   ██║   ███████║")
    print("   ╚═════╝ ╚══════╝  ╚═╝  ╚═╝   ╚═╝   ╚══════╝\033[0m")
    print("\033[1;34mC       L      A       T       S       E       C       U       R       E\033[0m   \033[1;31m(Version 1.01)\033[0m")
    author = "🛡️ By Josh Clatney - Ethical Pentesting Enthusiast 🛡️"
    Write.Print(author + "\n[C.S]\nClatSecure AES Key Generator\n", Colors.white, interval=0)
    user_password = input("Enter password for key derivation: ").encode()
