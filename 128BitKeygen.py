import os
import numpy as np
from argon2.low_level import hash_secret_raw, Type

# AES 128 uses 10 rounds (plus 1 initial round key addition step)
ROUND_COUNT = 10
KEY_SIZE = 16  # 16 bytes for AES-128

# AES S-Box
S_BOX = [
    
]

# Rcon used in key expansion
RCON = [
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
]

def generate_aes_key(password: bytes, salt: bytes = None):
    if salt is None:
        salt = os.urandom(16)  # 16-byte salt
    try:
        # Using Argon2i to derive a 16-byte key (AES-128)
        key = hash_secret_raw(
            secret=password,
            salt=salt,
            time_cost=2,       # number of iterations
            memory_cost=102400,# 100 MB
            parallelism=8,     # number of parallel threads
            hash_len=KEY_SIZE, # 16 bytes
            type=Type.I        # Argon2i variant
        )
        return key, salt
    except Exception as e:
        raise ValueError(f"Key derivation failed: {str(e)}")

def sub_word(word):
    return [S_BOX[b] for b in word]

def rot_word(word):
    return word[1:] + word[:1]

def key_expansion(key):
    expanded_keys = []
    rcon_iter = 1

    # Initial 4 words (16 bytes) from the original key
    for i in range(KEY_SIZE // 4):  # 16 bytes / 4 = 4 words
        expanded_keys.append([key[4 * i], key[4 * i + 1], key[4 * i + 2], key[4 * i + 3]])

    # Each round key: 4 words -> total 44 words for AES-128 (10 rounds + 1 initial)
    for i in range(KEY_SIZE // 4, 4 * (ROUND_COUNT + 1)):
        temp = expanded_keys[-1][:]
        # Word rotation and Rcon apply every 4 words
        if i % (KEY_SIZE // 4) == 0:
            temp = xor_words(sub_word(rot_word(temp)), [RCON[rcon_iter - 1], 0, 0, 0])
            rcon_iter += 1
        expanded_keys.append(xor_words(expanded_keys[i - (KEY_SIZE // 4)], temp))

    # Reshape into round-key blocks: (ROUND_COUNT+1) x 4 x 4
    return np.array(expanded_keys).reshape((ROUND_COUNT + 1, 4, 4))

def xor_words(word1, word2):
    return [a ^ b for a, b in zip(word1, word2)]

def add_round_key(state, round_key):
    return np.bitwise_xor(state, round_key)

def sub_bytes(state):
    return np.vectorize(lambda x: S_BOX[x])(state)

def shift_rows(state):
    state[1] = np.roll(state[1], -1)
    state[2] = np.roll(state[2], -2)
    state[3] = np.roll(state[3], -3)
    return state

# Note: This mix_columns is illustrative, not standard AES GF(2^8)
def mix_columns(state):
    for i in range(4):
        col = state[:, i]
        state[:, i] = np.roll(col, 1)
    return state

def aes_round(state, round_key):
    state = sub_bytes(state)
    state = shift_rows(state)
    state = mix_columns(state)
    state = add_round_key(state, round_key)
    return state

def final_round(state, round_key):
    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, round_key)
    return state

def generate_and_print_keys(password: bytes):
    for _ in range(100):  # Generate 100 keys
        try:
            generated_key, used_salt = generate_aes_key(password)
            round_keys = key_expansion(generated_key)
            state = np.random.randint(0, 256, (4, 4), dtype=np.uint8)

            # Initial round key addition
            state = add_round_key(state, round_keys[0])

            # 9 main rounds for AES-128
            for round_num in range(1, ROUND_COUNT):
                state = aes_round(state, round_keys[round_num])

            # Final round (10th)
            state = final_round(state, round_keys[ROUND_COUNT])

            # Output final state as hexadecimal for readability
            final_state_hex = ''.join(
                [''.join([f"{byte:02X}" for byte in row]) for row in state]
            )
            print(f"Final state (hexadecimal):\n{final_state_hex}")

        except ValueError as ve:
            print(ve)

if __name__ == "__main__":
    user_password = input("Enter password: ").encode()
    generate_and_print_keys(user_password)
    input("Press Enter to exit...")